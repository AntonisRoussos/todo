class ExpensesController < ApplicationController
  ...
  def check_date_due_from
    fmt=I18n.t('date.formats.default')
    respond_to do |format|
          if date_is_valid?(fmt, params[:dateoccuredfrom]) 
     		a = format.json { render :json => true }
          else
     		a = format.json { render :json => false }
          end 
  ...
  def check_date_due_to
    fmt=I18n.t('date.formats.default')
    respond_to do |format|
          if date_is_valid?(fmt, params[:dateoccuredto]) 
     		a = format.json { render :json => true }
          else
     		a = format.json { render :json => false }
          end 
...
#		  if @expenses.count = 0 then render 'noexpenses', :content_type => 'text/html'  end
		  if @expenses.count > 0 then render 'shownew', :content_type => 'text/html' else render 'noexpenses', :content_type => 'text/html'  end
	     end
	  }
          format.json  { render :json => @tasks }
 ...
 def getChartdata
    ...
    barchartdata = Array.new

    fmt=I18n.t('date.formats.default')
    datefrom = Date.strptime(str=params[:dateoccuredfrom],fmt)
    dateto = Date.strptime(str=params[:dateoccuredto],fmt)
    eitype = params[:ttype]
    ...
    @expenses.each do |expense|
             barchartdata.push(expense.cat_sum, expense.cat_desc)
    end
...
def exportToExcel
    ...
    respond_to do |format|
      format.html # exportToExcel.html.erb
      format.xls
      format.xml  { render :xml => @expenses }
    end
  end
